...
create table account_entity (account_id integer not null auto_increment, account_number varchar(255), account_type varchar(255), active bit not null, current_balance decimal(20,10), user_id integer, initial_balance decimal(20,10), updated_at datetime(6), primary key (account_id)) engine=InnoDB;
...
alter table account_entity add constraint FK4edjqj899qfrgsvxrsin43dpp foreign key (user_id) references user_entity (user_id);
...
-- e2e Data:
insert into account_entity (account_number,account_type,active,current_balance,user_id,initial_balance,updated_at,account_id) values
