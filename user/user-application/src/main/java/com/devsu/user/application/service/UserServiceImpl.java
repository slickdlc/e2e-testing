...
import com.devsu.user.domain.repository.UserRepository;
...
public class UserServiceImpl implements UserService {
  ...
  public void doIfExists(final Integer userId, final Consumer<User> consumerUser) {
    this.userRepository.findUserById(userId)
        .ifPresentOrElse(
            consumerUser,
            () -> {
              throw new ServiceException(HttpStatus.NOT_FOUND, String.format("El cliente con id [%s] no se ha encontrado", userId));
            });
  }
  ...
  public User getUserById(final Integer userId) {
    return this.userRepository.findUserById(userId)
        .orElseThrow(
            () -> new ServiceException(HttpStatus.NOT_FOUND, String.format("El cliente con id [%s] no se ha encontrado", userId)));
  }
